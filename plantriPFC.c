...
Generate connected simple spherical planar 4-regular graphs 
such that the vertex type is one of 3335,3334,3333,3344,3345, 3444, and 3336.


For the synposis, execute a command

         plantriPFC --help


This program is obtained from Brinkmann-MacKay's program plantri by adding

      VERTEX Inverse_Medial[NUMEDGES][NUMEDGES];
      int medial_count;			// number of medial graphs
      int is_PFC_everywhere(void)
      char trans(VERTEX f, int rootV, VERTEX Inverse_MedialArray[NUMEDGES][NUMEDGES])
      int inverse_medial_aux(VERTEX f0, int rootV, VERTEX Inverse_Medial[NUMEDGES][NUMEDGES], int nf)
      int write_inverse_medial(VERTEX f, char* path, int nf)

and by modifying

      static void write_dual_alpha(FILE *f, int doflip)

Modified by Yohji Akama (2019/9/23) when he visited Math. Dept., Fudan University.

Modified by Yohji Akama (2019/11/26) when he visited Math. Dept., Fudan University.

Comment added by Yohji Akama (2019/12/15) when he is in Math. Dept., Tohoku University.
...
#define PFCVERSION "2  - Dec 15, 2019"
...
#ifdef PFC
#define HELPMESSAGE (fprintf(stderr,"%s -a -c2 -q V\n\tgenerates all 2-conn. planar quadrangulations of V vertices s.t. each vertex type of the dual graph sums up to at most 15, up to isomorphism as embedding.\n\tEach graph has two output lines.\n\tThe first line lists the number of vertices of 7 types (3^3.5), (3^3.4), (3^4), (3^2.4^2), (3^2.4.5), (3.4^3), (3^3.6) and #V, f3, f4, f5, f6.\n\tThe second line is an adjacency list.\n\n%s -a -c2 -d -q V\n\tgenerates the duals of all 2-conn. planar quadrangulations of V vertices s.t. each vertex type of the dual graph sums up to at most 15, up to isomorphism as embedding.\n\tThe output format is as above.\n\tIn other words, it generates all 4-edge-conn. simple quartic planar graphs of V-2 vertices w/ Forman curv>0 everywhere, up to isomorphism as embedding.\n\n For <COUNT>-th graph, an inverse medial graph of it and the dual are written in ASCII format to <V-2>_<COUNT>.txt.\nThe first line is the graph of fewer vertices.\n\n",argv[0],argv[0]))
int seq_type[MAXF][4];	   /* the sequence types of the faces */
#endif
...
is_PFC(int* ca)
{
  return (      (ca[0]==3 && ca[1]==3 && ca[2]==3 && ca[3]==5)
	     || (ca[0]==3 && ca[1]==3 && ca[2]==3 && ca[3]==4)
	     || (ca[0]==3 && ca[1]==3 && ca[2]==3 && ca[3]==3)
	     || (ca[0]==3 && ca[1]==3 && ca[2]==4 && ca[3]==4)
	     || (ca[0]==3 && ca[1]==3 && ca[2]==4 && ca[3]==5)
	     || (ca[0]==3 && ca[1]==4 && ca[2]==4 && ca[3]==4)
	     || (ca[0]==3 && ca[1]==3 && ca[2]==3 && ca[3]==6)
		);
...
char
trans(VERTEX f, int rootV, VERTEX Inverse_MedialArray[NUMEDGES][NUMEDGES])
{
  int k;

  for(k=0;k<rootV;k++)
    if (f==Inverse_MedialArray[k][0]) return(k+'a');
...
int
inverse_medial_aux(VERTEX f0, int rootV, VERTEX Inverse_Medial[NUMEDGES][NUMEDGES],int nf)
{
  int j,k,l;
  EDGE* e;
  int rtV,tmp_rtV;
  ...
  for(j=0;j<rtV&&j<nf;j++)
    if(f0==Inverse_Medial[j][0]){
#ifdef DEBUG
      printf("f0=%d\tInverse_Medial[%d][0]=%d\n",f0,j,Inverse_Medial[j][0]);
#endif
      return rtV;}

  /*  for(k=0;k<rootV;k++){
    for(l=0;Inverse_Medial[k][l]!=-1;l++)
      printf("%d\t", Inverse_Medial[k][l]);
    printf("\n");
    }
  */
  ...
  e=firstedge[f0];
  Inverse_Medial[rtV][j++]=f0;
  #ifdef DEBUG
  printf("%d(degree=%d) :", f0,degree[f0]);
  #endif

  for(;j<=degree[f0]; j++,e=e->next){
    Inverse_Medial[rtV][j]=e->invers->prev->end;
    #ifdef DEBUG
    printf("%d\t",    Inverse_Medial[rtV][j]);
    #endif
  }
  #ifdef DEBUG
  printf("\n");
  ...
  for(j=1;j<=degree[f0]; j++,e=e->next){
      rtV=inverse_medial_aux(Inverse_Medial[tmp_rtV][j],rtV,Inverse_Medial,nf);
  }
  Inverse_Medial[rtV][j]=-1;

  return(rtV);
} /* end of inverse_medial_aux() */


void write_adj_to_file(char* path, int rootV)
{
  ...
  for(i=0;i<rootV&&i<nvlim;i++){
    for(l=0; l<degree[Inverse_Medial[i][0]]; l++)
      fprintf(fp,"%c",trans(Inverse_Medial[i][1+l],rootV,Inverse_Medial));
    //            fprintf(fp,"%d",Inverse_Medial[i][1+l]);
    if (i<rootV-1) fprintf(fp,"%c",',');
    else fprintf(fp,"\n");
  }
...
int
write_inverse_medial(VERTEX f, char* path,int nf)
/* Write an adj list of an inverse medial graph having f as a vertex to path.
The return value is the number of the vertices of an inverse medial graph */
{
  ...
  for(i=0;i<nf;i++)
    for(l=0;l<nf;l++)
      Inverse_Medial[i][l]=-1;

  rootV=inverse_medial_aux(f,0,Inverse_Medial,nf);
  write_adj_to_file(path, rootV);

  return(rootV);
} /* end of write_inverse_medial() */



    
int
is_PFC_everywhere(void)
{
    register int i,nf,sz;
    int j;
    int nvlim;
    int canonical[4];
    int cmp_canonical_vtx_typ();
    int n2,n3,n4,n5,n6,n7,n8,f_3,f_4,f_5,f_6;
    int Enum,Vnum;
    ...
    for (i = 0; i < nvlim; ++i)
    {
		...
		cmp_canonical_vtx_typ(nf, canonical);
		if (is_PFC(canonical)!=1){
    ...
    for(i=0; i<nf; i++){
      cmp_canonical_vtx_typ(i,canonical);
      if(canonical[0]==3 && canonical[1]==3 && canonical[2]==3 && canonical[3]==5) n2++;
      if(canonical[0]==3 && canonical[1]==3 && canonical[2]==3 && canonical[3]==4) n3++;
      if(canonical[0]==3 && canonical[1]==3 && canonical[2]==3 && canonical[3]==3) n4++;
      if(canonical[0]==3 && canonical[1]==3 && canonical[2]==4 && canonical[3]==4) n5++;
      if(canonical[0]==3 && canonical[1]==3 && canonical[2]==4 && canonical[3]==5) n6++;
      if(canonical[0]==3 && canonical[1]==4 && canonical[2]==4 && canonical[3]==4) n7++;
      if(canonical[0]==3 && canonical[1]==3 && canonical[2]==3 && canonical[3]==6) n8++;
    }
    f_6=n8/6;
    f_5=(n2+n6)/5;
    f_3=n2 + n3 + ( 4*n4 + 2*n5 + 2*n6 +   n7)/3 + n8;
    //    f_3=f_5+8+n8;    // in case n8 = 0
    f_4=(    n3          + 2*n5 +   n6 + 3*n7)/4;
    //    f_4=nf-2*f_5-6;	/* F=V+2. */ // in case n8=0
    printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t|\t%d\t%d\t%d\t%d\t%d\n",n2,n3,n4,n5,n6,n7,n8,nf,f_3,f_4,f_5,f_6);


    medial_count++;

    system("[ -d InverseMedial ] || mkdir InverseMedial");
    Enum=n2+n3+n4+n5+n6+n7+n8;
    sprintf(path, "InverseMedial/%d_%d.txt", Enum, medial_count);
        
    
    Vnum=write_inverse_medial(0,path,nf);


    if (Enum+2-2*Vnum>0)	/* Vnum < Fnum */
      write_inverse_medial(firstedge[0]->end,path,nf);
    else{
      remove(path);
      write_inverse_medial(firstedge[0]->end,path,nf);
      write_inverse_medial(0,path,nf);
    }
      

    return 1;
} /* end of is_PFC_everywhere */

int
cmp_canonical_vtx_typ(int f, int* canonical)
{
  int i,j,temp;

 for(j=0;j<facesize[f];j++)
     canonical[j]=seq_type[f][j];

  for(i=0; i<facesize[f]; i++){
    for(j=facesize[f]-1; j>i; j--){
      if(canonical[j] < canonical[j-1]){
        temp = canonical[j];
...
#ifdef PFC
    if(!is_PFC_everywhere()){
      //      fprintf(stderr,"Not PFC!\n");
      return;
    }
#endif
   ...
   purpose is to write the graph and to collect some stats. */
{
    ...
    if (pswitch)
    {
	if (oswitch) ADDBIG(nout_e_op[ne/2],wt);
...
*/
{
	...
	if (numnft == 0)
        {
	    if (pswitch) startpolyscan(nbtot,nbop);
            else         got_one(nbtot,nbop,3+(numnft==0));
        }
        return;
    }
...
main(int argc, char *argv[])
{
    ...
    {
      fprintf(stderr,"PFC extension version %s\n",PFCVERSION);
