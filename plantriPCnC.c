#define PCnCVERSION "2  - Sep 30, 2019"

/* plantriPCnC.c :

 generate connected simple spherical planar graphs such that the corner curvutre is positive everywhere and the number of vertices is at most the number of faces. 

Compile this program with

        gcc plantriPCnC.c -o plantriPCnC 

Then you will obtain an executable file plantriPCnC
...
int is_PCnC_everywhere(void)
int is_PCnC_dual_everywhere(void)
...
Modified by Yohji Akama (2019/9/30) at Math. Dept., Fudan University.
*/

#define HELPMESSAGE (fprintf(stderr,"%s -a -c2 -f5 -p V\n\tgenerates all 2-conn. simple planar graph of V vertices s.t. the corner curv>0 everywhere and V <= # of faces, up to graph isomorphism.\n%s -a -c2 -d -q V\n\tgenerates the duals of such.\n\n",argv[0],argv[0]))
...
int
is_PCnC_everywhere(void)

/* decides whether 
(a) the vertex degree of the graph is 3,4 or 5;
(b) the  corner curvature ( 1/deg(v) + 1/deg(f) - 1/2, (v,f) is a corner )
    is positive everywhere; and
(c) # of vertices is less than or equal to # of faces.
*/
{
    register int i,nf,sz;
    int j;
    ...
    for(i=0;i<nvlim;i++){
      ...
      if (degree[i]==3){
	for(j=0,e=firstedge[i]; j<degree[i]; j++,e=e->next){
	  sz = facesize[e->rightface];
	  if ( sz < 3 || 5 < sz ) return 0;
	}
      } else if (degree[i] == 4 || degree[i] == 5 ){
	for(j=0,e=firstedge[i]; j<degree[i]; j++,e=e->next){
	  if ( facesize[e->rightface] != 3) return 0;
	}
      } else
	return 0;
    }
...
*/
{
    ...
    for(i=0;i<nf;i++){
      ...
      if (facesize[i]==3){
	for(j=0,e=facestart[i]; j<facesize[i]; j++,e=e->invers->prev){
	  sz = degree[e->start];
	  if ( sz < 3 || 5 < sz ) return 0;
	}
      } else if (facesize[i] == 4 || facesize[i] == 5 ){
	for(j=0,e=facestart[i]; j<facesize[i]; j++,e=e->invers->prev){
	  if ( degree[e->start] != 3) return 0;
	}
      } else
	return 0;
    }
   ...
   and if doflip != 0 also write in prev direction. */
{
    ...
    if (is_PCnC_everywhere() != 1 ) return;
...
main(int argc, char *argv[])
{
    ...
    {
      fprintf(stderr,"PCnC extension version %s\n",PCnCVERSION);
