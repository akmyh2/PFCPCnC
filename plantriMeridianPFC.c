...
Modified by Yohji Akama (2019/11/26) when he visited Math. Dept., Fudan University.a
*/
  
#define PFCVERSION "2  - Nov 26, 2019"
...
#ifdef PFC
#define HELPMESSAGE (fprintf(stderr,"%s -a -c2 -q V\n\tgenerates all 2-conn. planar quadrangulations of V vertices s.t. each vertex type of the dual graph sums up to at most 15, up to graph isomorphism.\n\tEach graph has two output lines.\n\tThe first line lists the number of vertices of 7 types (3^3.5), (3^3.4), (3^4), (3^2.4^2), (3^2.4.5), (3.4^3), (3^3.6) and #V, f3, f4, f5, f6.\n\tThe second line is an adjacency list.\n\n%s -a -c2 -d -q V\n\tgenerates the duals of all 2-conn. planar quadrangulations of V vertices s.t. each vertex type of the dual graph sums upt to at most 15, up to graph isomorphism.\n\tThe output format is as above.\n\tIn other words, it generates all 4-edge-conn. simple quartic planar graphs of V-2 vertices w/ Forman curv>0 everywhere, up to graph isomorphism.\n\n For <COUNT>-th graph, an inverse meridian graph of it and the dual are written in ASCII format to V-2_<COUNT>.txt.\n\n",argv[0],argv[0]))
...
typedef int VERTEX;
VERTEX Inverse_Meridian[NUMEDGES][NUMEDGES];

int meridian_count;			/* number of meridian graphs */
...
char
trans(VERTEX f, int rootV, VERTEX Inverse_MeridianArray[NUMEDGES][NUMEDGES])
{
  int k;

  for(k=0;k<rootV;k++)
    if (f==Inverse_MeridianArray[k][0]) return(k+'a');
...
int
inverse_meridian_aux(VERTEX f0, int rootV, VERTEX Inverse_Meridian[NUMEDGES][NUMEDGES],int nf)
{
  int j,k,l;
  EDGE* e;
  int rtV,tmp_rtV;
  ...
  for(j=0;j<rtV&&j<nf;j++)
    if(f0==Inverse_Meridian[j][0]){
#ifdef DEBUG
      printf("f0=%d\tInverse_Meridian[%d][0]=%d\n",f0,j,Inverse_Meridian[j][0]);
#endif
      return rtV;}

  /*  for(k=0;k<rootV;k++){
    for(l=0;Inverse_Meridian[k][l]!=-1;l++)
      printf("%d\t", Inverse_Meridian[k][l]);
    printf("\n");
    }
  */
  ...
  e=firstedge[f0];
  Inverse_Meridian[rtV][j++]=f0;
  #ifdef DEBUG
  printf("%d(degree=%d) :", f0,degree[f0]);
  #endif

  for(;j<=degree[f0]; j++,e=e->next){
    Inverse_Meridian[rtV][j]=e->invers->prev->end;
    #ifdef DEBUG
    printf("%d\t",    Inverse_Meridian[rtV][j]);
    #endif
  }
  #ifdef DEBUG
  printf("\n");
  ...
  for(j=1;j<=degree[f0]; j++,e=e->next){
      rtV=inverse_meridian_aux(Inverse_Meridian[tmp_rtV][j],rtV,Inverse_Meridian,nf);
  }
  Inverse_Meridian[rtV][j]=-1;

  return(rtV);
} /* end of inverse_meridian_aux() */


int
inverse_meridian(VERTEX f, char* path,int nf)
/* Inverse meridian graph construction. 
The return value is the number of the vertices of an inverse meridian graph */
{
  ...
  for(i=0;i<nf;i++)
    for(l=0;l<nf;l++)
      Inverse_Meridian[i][l]=-1;

  fp=fopen(path, "a");
  rootV=inverse_meridian_aux(f,0,Inverse_Meridian,nf);
  fprintf(fp,"%d ",rootV);

  for(i=0;i<rootV&&i<nvlim;i++){
    for(l=0; l<degree[Inverse_Meridian[i][0]]; l++)
      fprintf(fp,"%c",trans(Inverse_Meridian[i][1+l],rootV,Inverse_Meridian));
    //            fprintf(fp,"%d",Inverse_Meridian[i][1+l]);
...
is_PFC_everywhere(void)
{
    ...
    system("[ -d InverseMeridian ] || mkdir InverseMeridian");
    sprintf(path, "InverseMeridian/%d_%d.txt", n2+n3+n4+n5+n6+n7+n8, meridian_count);
        
    inverse_meridian(0,path,nf);
    inverse_meridian(firstedge[0]->end,path,nf);
